...
export class InputKeyNodeParser extends NodeParser {
  ...
  constructor() {
    super({
      InputKey: (node: InputKeyNode, value: string) => {
        node.inputKey = value
      },
      bControl: (node: InputKeyNode, value: string) => {
        node.control = value.toLowerCase() === 'true'
      },
      bAlt: (node: InputKeyNode, value: string) => {
        node.alt = value.toLowerCase() === 'true'
      },
      bShift: (node: InputKeyNode, value: string) => {
        node.shift = value.toLowerCase() === 'true'
      },
      bCommand: (node: InputKeyNode, value: string) => {
        node.command = value.toLowerCase() === 'true'
      },
    })
  }
  ...
  public parse(data: ParsingNodeData): NodeControl {
    ...
    if (inputKeyNode.control) modifiers.push('Ctrl')
    if (inputKeyNode.command) modifiers.push('Cmd')
    if (inputKeyNode.alt) modifiers.push('Alt')
    if (inputKeyNode.shift) modifiers.push('Shift')
