...
export class MacroInstanceNodeParser extends NodeParser {
  ...
  public parse(data: ParsingNodeData): NodeControl {
    ...
    const headlessNode = MacroInstanceNodeParser._HEADLESS_NODES.find(
      (n) => n.name === macroNode.macroGraphReference?.macroFuncName,
    )
    if (headlessNode) {
  ...
  private static getIconForMacro(macroFuncName: string): string {
    switch (macroFuncName) {
      case 'ForLoop':
        return IconLibrary.LOOP
      case 'ForLoopWithBreak':
        return IconLibrary.LOOP
      case 'WhileLoop':
        return IconLibrary.LOOP
      case 'Gate':
        return IconLibrary.GATE
      case 'FlipFlop':
        return IconLibrary.FLIPFLOP
      case 'Do':
        return IconLibrary.DO_N
      case 'DoOnce':
        return IconLibrary.DO_ONCE
      case 'IsValid':
        return IconLibrary.IS_VALID
      default:
        return IconLibrary.MACRO
    }
  }
}
