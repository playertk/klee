...
export class CallFunctionNodeParser extends NodeParser {
  ...
  constructor() {
    super({
      bIsPureFunc: (node: CallFunctionNode, value: string) => {
        node.isPureFunc = value === 'True'
      },
      bIsConstFunc: (node: CallFunctionNode, value: string) => {
        node.isConstFunc = value === 'True'
      },
      FunctionReference: (node: CallFunctionNode, value: string) => {
        ...
        } else if (node.functionReference?.memberParent?.className) {
          // && node.functionReference?.memberParent?.type !== 'Class'
  ...
  public parse(data: ParsingNodeData): NodeControl {
    ...
    node.backgroundColor =
      node.isPureFunc === true ? Constants.DEFAULT_FUNC_PURE_BACKGROUND_COLOR : Constants.DEFAULT_FUNC_BACKGROUND_COLOR
