...
export class GenericNodeParser extends NodeParser {
  ...
  constructor() {
    super({
      NodeGuid: (node: Node, v: string) => {
        node.guid = v
      },
      NodePosX: (node: Node, v: string) => {
        node.pos.x = Number.parseInt(v)
      },
      NodePosY: (node: Node, v: string) => {
        node.pos.y = Number.parseInt(v)
      },
      AdvancedPinDisplay: (node: Node, v: string) => {
        node.advancedPinDisplay = v === 'Shown' ? true : false
      },
      EnabledState: (node: Node, v: string) => {
        node.enabledState = v
      },
      ErrorType: (node: Node, v: string) => {
        node.errorType = Number.parseInt(v)
      },
      ErrorMsg: (node: Node, v: string) => {
        node.errorMsg = v.replace(/["]/g, '').replace(/\\\'/g, "'")
      },
    })
  }
  ...
  public parse(data: ParsingNodeData): NodeControl {
    ...
    if (!particularImplementation) {
      console.info(
        `There is no particular implementation for class ${data.node.class}. Falling back to the generic node class.`,
      )
  ...
  private hideExecPins(node: Node): void {
    ...
    for (const property of node.customProperties) {
      ...
      if (property.direction == undefined || property.category !== PinCategory.exec) {
        continue
      }
