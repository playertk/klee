...
export class PinPropertyParser implements CustomPropertyParser {
  ...
  private static readonly _ATTRIBUTE_PARSERS: {
    ...
    PinId: (p: PinProperty, value: string) => {
      p.id = value
    },
    PinName: (p: PinProperty, value: string) => {
      p.name = prettifyText(BlueprintParserUtils.parseString(value))
    },
    PinFriendlyName: (p: PinProperty, value: string) => {
      p.friendlyName = prettifyText(PinPropertyParser.parsePinFriendlyName(value))
    },
    'PinType.PinCategory': (p: PinProperty, value: string) => {
      p.category = PinPropertyParser.parsePinCategory(value)
    },
    Direction: (p: PinProperty, value: string) => {
      p.direction = PinPropertyParser.parseDirection(value)
    },
    PinToolTip: (p: PinProperty, value: string) => {
      p.toolTip = BlueprintParserUtils.parseString(value)
    },
    'PinType.PinSubCategory': (p: PinProperty, value: string) => {
      p.subCategory = BlueprintParserUtils.parseString(value)
    },
    'PinType.PinSubCategoryObject': (p: PinProperty, value: string) => {
      p.subCategoryObject = PinPropertyParser.parseSubCategoryObject(value)
    },
    'PinType.bIsReference': (p: PinProperty, value: string) => {
      p.isReference = value.toLowerCase() === 'true'
    },
    'PinType.bIsConst': (p: PinProperty, value: string) => {
      p.isConst = value.toLowerCase() === 'true'
    },
    'PinType.bIsWeakPointer': (p: PinProperty, value: string) => {
      p.isWeakPointer = value.toLowerCase() === 'true'
    },
    'PinType.bIsUObjectWrapper': (p: PinProperty, value: string) => {
      p.isUObjectWrapper = value.toLowerCase() === 'true'
    },
    ...
    AutogeneratedDefaultValue: (p: PinProperty, value: string) => {
      p.autogeneratedDefaultValue = BlueprintParserUtils.parseString(value)
    },
    LinkedTo: (p: PinProperty, value: string) => {
      p.linkedTo = PinPropertyParser.parseLinkedTo(value)
    },
    bHidden: (p: PinProperty, value: string) => {
      p.hidden = value.toLowerCase() === 'true'
    },
    bDefaultValueIsIgnored: (p: PinProperty, value: string) => {
      p.defaultValueIsIgnored = value.toLowerCase() === 'true'
    },
    bDefaultValueIsReadOnly: (p: PinProperty, value: string) => {
      p.defaultValueIsReadOnly = value.toLowerCase() === 'true'
    },
    bAdvancedView: (p: PinProperty, value: string) => {
      p.advancedView = value.toLowerCase() === 'true'
    },
    bOrphanedPin: (p: PinProperty, value: string) => {
      p.orphanedPin = value.toLowerCase() === 'true'
    },
    bNotConnectable: (p: PinProperty, value: string) => {
      p.notConnectable = value.toLowerCase() === 'true'
    },
    PersistentGuid: (p: PinProperty, value: string) => {
      p.persistentGUID = BlueprintParserUtils.parseString(value)
    },
    'PinType.ContainerType': (p: PinProperty, value: string) => {
      p.containerType = value as PinContainerType
    },
  ...
  parse(propertyData: string, nodeName: string): PinProperty {
    ...
    const matches = propertyData.matchAll(
      /([a-zA-Z0-9_.]+)\s*=\s*(("[^"]*")|(\([^)]*\))|(\w*\(\w*(?:[^(]*\([^)]*(?:"[^"]*")\))*\))|(\w*\([^)]*\([^)]*\)[^)]*\))|(\w*\([^)]*\))|([^,]*))/g,
    )
  ...
  private static parseDirection(value: string): PinDirection {
    ...
    switch (value) {
      case 'EGPD_Output':
        return PinDirection.EGPD_Output
      case 'EGPD_Input':
      default:
    ...
    switch (p.category) {
      case PinCategory.float:
        return {
          ...
          data: removeInsignificantTrailingZeros(BlueprintParserUtils.parseString(value)),
        }
      ...
      case PinCategory.byte:
        if (p.subCategoryObject.type === 'Enum') {
          return {
            control: TextBoxControl,
            data: BlueprintParserUtils.parseEnumValue(p.subCategoryObject.class, value),
          }
        } else {
  ...
  private static parseDefaultValueStruct(subCategoryObject: string, value: string): { data: any; control: any } {
    switch (subCategoryObject) {
      ...
      case StructClass.LINEAR_COLOR:
        ...
        const color = new Color((cParams[0] || 0) * 255, (cParams[1] || 0) * 255, (cParams[2] || 0) * 255, cParams[3])
        color.applyGamma()
