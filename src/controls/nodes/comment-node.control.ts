...
export class CommentNodeControl extends NodeControl implements DrawableControl {
  ...
  public onDraw(canvas: Canvas2D) {
    canvas
      .fillStyle(this.bodyBackgroundColor)
  ...
  private applyCommentColor(nodeColor: string) {
    if (nodeColor == undefined || nodeColor == '') nodeColor = CommentNodeControl._DEFAULT_COMMENT_COLOR
  ...
  private drawTitle(canvas: Canvas2D) {
    ...
    canvas
      .fillStyle(this.headerBackgroundColor)
      .roundedRectangle(0, 0, this.width, headerHeight, {
        radiusTopLeft: 5,
        radiusTopRight: 5,
        radiusBottomLeft: 0,
        radiusBottomRight: 0,
