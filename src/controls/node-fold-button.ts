...
export class NodeFoldButton extends InteractableUserControl implements InteractableControl {
  ...
  onMouseUp(ev: MouseEvent): boolean {
    ...
    if (this.onClick) this.onClick(this.foldoutState)
  ...
  onDraw(canvas: Canvas2D): void {
    ...
    /// #endif
    if (this.hovered) {
      canvas.fillStyle('rgba(255,255,255,0.1)').fillRect(0, 0, this.size.x, this.size.y)
    }
  ...
  drawChevron(canvas: Canvas2D) {
    ...
    canvas.fillStyle('#eee').beginPath()
    if (this.foldoutState) {
      canvas.moveTo(5, 5).lineTo(0, -2).lineTo(-5, 5).lineTo(5, 5)
    } else {
      canvas.moveTo(5, -2).lineTo(0, 5).lineTo(-5, -2).lineTo(5, -2)
    }
