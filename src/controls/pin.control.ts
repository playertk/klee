...
export class PinControl extends UserControl {
  ...
  override initialize() {
    if (this.visible) {
      if (!this.pinProperty.hidden && !this.pinProperty.hideName) {
        this.width =
          this.formattedNameWidth(this.pinProperty) + PinControl.PINS_PADDING_HORIZONTAL + PinControl.PIN_ICON_WIDTH
  ...
  private initPinIcons() {
    switch (this._pinProperty.containerType) {
      case PinContainerType.Array:
        this.icon = this._pinProperty.isLinked
          ? new Path2D(IconLibrary.PIN_ARRAY_CONNECTED)
          : new Path2D(IconLibrary.PIN_ARRAY_DISCONNECTED)
        break
    ...
    if (this.icon === undefined && this._pinProperty.isReference) {
      if (this._pinProperty.isLinked) this.icon = new Path2D(IconLibrary.PIN_REFERENCE_CONNECTED)
      else this.icon = new Path2D(IconLibrary.PIN_REFERENCE_DISCONNECTED)
    }
  }
  ...
  public postInit(): void {
    if (this.pinProperty.shouldDrawDefaultValueBox && this._pinProperty.defaultValueControlClass) {
      ...
      this.defaultValueBox.position.x =
        this.formattedNameWidth(this._pinProperty) +
        PinControl.PINS_PADDING_HORIZONTAL +
        PinControl.PINS_PADDING_LEFT_DEFAULT_BOX
  ...
  drawPinIcon(canvas: Canvas2D, icon: Path2D) {
    ...
    if (this.secondaryIcon !== undefined) {
      canvas.fillStyle(this._secondaryColor).fill(this.secondaryIcon, 'evenodd')
    }
  }
  ...
  private drawPin(canvas: Canvas2D) {
    ...
    if (this.icon === undefined) {
      canvas.fillStyle(this._color).fillCircle(pinX + 6, 0, 2.3)

      if (this._pinProperty.isLinked) {
        canvas.fillCircle(pinX, 0, 6)
      } else {
        canvas.strokeStyle(this._color).lineWidth(2).strokeCircle(pinX, 0, 4.8)

        this.drawDefaultValueBox(canvas)
      }

      canvas.strokeStyle('#000').lineWidth(0.5).strokeCircle(pinX, 0, 6)
    } else {
  ...
  private drawExecutionPin(canvas: Canvas2D) {
    ...
    if (this.pinProperty.isLinked) canvas.fill()
    else canvas.stroke()
  ...
  private setupTextDrawing(canvas: Canvas2D): number {
    ...
    canvas.font('12px sans-serif').fillStyle('#eee')
  ...
  public formattedNameWidth(pin: PinProperty): number {
    return (
      this.app.canvas.font(Constants.NODE_FONT).getContext().measureText(pin.formattedName).width +
      PinControl.PIN_NAME_PADDING_LEFT
    )
  }
  ...
  public getPinAbsolutePosition(): Vector2 {
    ...
    if (this.pinProperty.direction === PinDirection.EGPD_Output) {
      ...
      if (this.pinProperty.category === PinCategory.delegate) position.x += 8
    } else {
