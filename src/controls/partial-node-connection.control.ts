...
export class NodePartialConnectionControl extends Control implements DrawableControl {
  ...
  draw(canvas: Canvas2D): void {
    ...
    canvas.lineWidth(this._lineWidth).beginPath().setLineDash([30, 4, 2.5, 4, 2.5, 4, 2.5])

    if (this._isDirectionOutput) {
      canvas.moveTo(6, 0).lineTo(NodePartialConnectionControl.LINE_LENGTH, 0)
    } else {
      canvas.moveTo(-6, 0).lineTo(-NodePartialConnectionControl.LINE_LENGTH, 0)
    }

    canvas.strokeStyle(this._color).stroke()
