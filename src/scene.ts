...
export class Scene {
  ...
  private createConnectionLines() {
    ...
    for (let i = pins.length - 1; i >= 0; --i) {
      ...
      if (pin.pinProperty.direction != PinDirection.EGPD_Output) continue
    ...
    for (let i = pins.length - 1; i >= 0; --i) {
      ...
      if (connectedPins.indexOf(pin.pinProperty.getUniqueName()) >= 0) continue
  ...
  calculateCentroid(): Vector2 {
    ...
    if (this.nodes.length == 0) return centroid
